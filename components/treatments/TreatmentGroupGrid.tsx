...
/**
 * The three treatment groups — Face, Skin, Body & Wellness — rendered as
 * separate blocks with real air between them.
 *
 * Used by BOTH the homepage and the /treatments pillar page so the two can
 * never drift apart, and so both match the Treatments dropdown.
 *
 * Card styling is unchanged from the original flat grid; what changed is the
 * rhythm. Four cards per group sit on one row at desktop width, which is why
 * the grid is 4-up rather than 3-up.
 */
...
function TreatmentCard({ item }: { item: GroupedTreatment }) {
  ...
  // Should not occur while every published item has a page, but a missing
  // card must degrade to an enquiry rather than a broken link.
  if (!item.card) {
    return (
...
export default function TreatmentGroupGrid({
  headingLevel = "h2",
  showSectionCta = true,
}: {
  headingLevel?: "h1" | "h2";
