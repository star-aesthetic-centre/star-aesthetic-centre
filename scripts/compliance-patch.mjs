...
const RULES = [
  [/060 123 0000/g, "076 977 0386"],
  [/wa\.me\/27601230000/g, "wa.me/27769770386"],
  [/one of the most effective applications of botulinum toxin/gi,
   "a well-established, evidence-based use of botulinum toxin"],
  [/\bMBBch\b/g, "MBBCh"],
  [/\bMB, ?BS\b/g, "MBBCh"],
];
...
for (const table of ["treatments","site_pages"]) {
  ...
  for (const row of data ?? []) {
    ...
    for (const [k,v] of Object.entries(row)) {
      ...
      const src = isStr ? v : JSON.stringify(v);
      let out = src;
      for (const [re,to] of RULES) out = out.replace(re,to);
      if (out !== src) patch[k] = isStr ? out : JSON.parse(out);
    }
    if (!Object.keys(patch).length) continue;
    const keyCol = "id" in row ? "id" : "slug" in row ? "slug" : "key";
    const id = row[keyCol];
    console.log(`${DRY?"[dry] ":""}${table}/${id}: ${Object.keys(patch).join(", ")}`);
    if (!DRY) {
      const { error: e2 } = await sb.from(table).update(patch).eq(keyCol, id);
